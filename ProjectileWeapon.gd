...
export var max_angular_velocity: float = 0 setget set_max_angular_velocity, get_max_angular_velocity
var angular_velocity: Vector3 = Vector3(0,0,0) setget set_angular_velocity, get_angular_velocity
...
func is_a_turret() -> bool: return max_angular_velocity>1e-5
func is_ready() -> bool:
...
func turn(delta: float):
	if angular_velocity.length()>1e-5:
...
func shoot(ship_translation: Vector3, ship_rotation: float, ship_velocity: Vector3,
	...
	var weapon_rotation: float = ship_rotation
	if is_a_turret():
		weapon_rotation += rotation[1]
	var heading: Vector3 = Vector3(1,0,0).rotated(y_axis,weapon_rotation)
