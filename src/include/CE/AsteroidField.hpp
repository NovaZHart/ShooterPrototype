  ...
  namespace CE {
    ...
    class AsteroidField {
      ...
      struct AsteroidSearchResult {
        ...
        std::pair<AsteroidSearchResult,AsteroidSearchResult> without(const AsteroidSearchResult &region) const;

        // If the two regions overlap, merge them. Returns false if
        // they can't be merged. If they can be merged, returns true
        // and the merged range.
        std::pair<bool,AsteroidSearchResult> merge(const AsteroidSearchResult &one,const AsteroidSearchResult &two) const;
      };
      ...
      struct AsteroidLayer {
        ...
        bool theta_range_of_rect(Rect2 rect,vector<AsteroidSearchResult> &results);

      private:
        // Clears the asteroid field and generates a new one from the given selection of asteroids.
        // Called from constructor. Never call a second time, or pointers will be invalid.
        void create_asteroids(const AsteroidPalette &palette,CheapRand32 &rand);
